...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:5000/allroutes/singleproduct/${userid}`);
        setProductData(response.data.singleproduct);
        setFormData({
          productcode: response.data.singleproduct?.productcode,
          name: response.data.singleproduct?.name,
          price: response.data.singleproduct?.price,
          category: response.data.singleproduct?.category,
          quantity: response.data.singleproduct?.quantity,
          onhold: response.data.singleproduct?.onhold
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://127.0.0.1:5000/allroutes/updateproduct/${userid}`, formData);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      {productData ? (
        <div className="max-w-md w-full space-y-8">
          <div>
            <h1 className="text-center text-3xl font-extrabold text-gray-900">Update Product</h1>
          </div>
